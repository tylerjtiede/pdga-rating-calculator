...
class WhatIfPanel(ctk.CTkFrame):
    def __init__(self, master, on_change, **kwargs):
        ...
        self._target_entry = ctk.CTkEntry(
            target_row, placeholder_text="Target rating", width=120, height=32
        )
        self._target_entry.pack(side="left", padx=(0, 6))

        self._rounds_entry = ctk.CTkEntry(
            target_row, placeholder_text="# rounds", width=70, height=32
        )
    ...
    def _solve_target(self):
        ...
        if not target_str.isdigit() or not rounds_str.isdigit():
            ...
            return
        self._on_change(
            list(self._whatif_ratings),
            solve_target=int(target_str),
            solve_rounds=int(rounds_str),
        )

    def set_target_result(self, msg: str, success: bool):
        self._target_lbl.configure(
            text=msg, text_color=C_GREEN if success else C_RED
        )
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            topbar, text="PDGA Ratings Calculator",
